...
export async function POST(req: Request) {
  ...
  if (!sniffed || !ALLOWED.has(sniffed)) return Response.json({ ok: false, reason: "invalid_input", message: "That file's contents don't match a JPEG, PNG, WEBP or GIF image." }, { status: 400 });
  ...
  const r = await askClaudeJson<InspectionResult>({
    ...
    images: [{ media_type: sniffed as "image/jpeg", data }],
