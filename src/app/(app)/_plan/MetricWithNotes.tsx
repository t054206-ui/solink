 ...
 */
export function MetricWithNotes({ label, term, data, format, unit, className, extra }: {
  label: ReactNode; term?: string; data: Classified; format?: (v: number) => string; unit?: string; className?: string; extra?: ReactNode;
}) {
  const notes = data.notes ?? [];
  return (
    <div className={cn("flex flex-col gap-1.5 min-w-0", className)}>
      <Metric label={label} term={term} data={data} format={format} unit={unit} />
