...
export default async function MaintenancePage() {
  const [{ data: cases, mode }, { data: appointments }, { data: providers }, { data: systems }, { data: incidents }, settings] = await Promise.all([
    listMaintenance(), listAppointments(), listProviders(), listSystems(), listIncidents(), getPlatformSettings(),
  ]);
  ...
  return (
    <div>
      ...
      {system ? (
        ...
        <EmptyState title="Maintenance starts with your system">Once your system is recorded in the Solar Passport, cleanings, inspections and repairs are booked and tracked here.</EmptyState>
      )}

      {/* Maintenance costs are no longer shown on these pages (owner, 2026-09-24). The data model is unchanged. */}
      <div className="mt-6">
        <PredictiveCard signals={signals} systemId={system?.id} thresholds={thresholdsFact(settings)?.text ?? null} />
      </div>
    </div>
  );
}
...
function buildSignals(i: {
  ...
  if (trend.value === null) trend = unavailable(i.production.length === 0 ? "No production records yet." : "Fewer than 3 daily records in one of the windows.");
