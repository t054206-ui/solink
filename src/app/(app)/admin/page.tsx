...
import { ADMIN_SECTIONS } from "./_components/admin-sections";
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader title="Admin sections" subtitle="Each mirrors one or more database tables." />
            <CardBody>
              <ul className="grid gap-1 sm:grid-cols-2">
                {ADMIN_SECTIONS.filter((s) => s.href !== "/admin").map((s) => (
                  <li key={s.href}><Link href={s.href} className="flex items-center justify-between rounded-[10px] px-3 py-2 text-[13.5px] text-fg-secondary hover:bg-inset hover:text-fg">{s.label}<ArrowRight className="size-3.5 text-fg-muted" aria-hidden /></Link></li>
                ))}
              </ul>
            </CardBody>
          </Card>
