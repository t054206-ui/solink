...
import { listUserProfiles } from "../_lib/data";
...
export default async function UsersPage() {
  const [users, access] = await Promise.all([listUserProfiles(), getAdminAccess()]);
  return (
    <>
      <PageHeader eyebrow="Admin" title="Users" description="user_profiles rows: display name and role. Emails live in auth.users and are not listed here." />
      <div className="space-y-4">
        ...
        : (
          <Table caption="User profiles">
            <thead><tr><Th>User id</Th><Th>Name</Th><Th>Role</Th><Th>Provider company</Th><Th>Created</Th></tr></thead>
            <tbody>
              {users.data.map((u) => (
                <tr key={u.user_id}>
