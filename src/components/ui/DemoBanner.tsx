...
export function DemoBanner({ text = "DEMO DATA: NOT REAL", className, detail }: { text?: string; className?: string; detail?: string }) {
  return (
    <div role="note" className={cn("demo-stripe flex items-start gap-2 rounded-[var(--radius)] border border-[var(--cls-demo)]/55 bg-[var(--cls-demo-soft)] px-2.5 py-1.5 text-[12.5px] text-[var(--critical-fg)]", className)}>
