...
export function DemoBanner({ text = "DEMO DATA — NOT REAL", className, detail }: { text?: string; className?: string; detail?: string }) {
  return (
    <div role="note" className={cn("demo-stripe flex items-start gap-2 rounded-[10px] border border-[var(--cls-demo)]/50 bg-[var(--cls-demo-soft)] px-3 py-2 text-[13px] text-[var(--critical-fg)]", className)}>
