...
function Shell({ icon: Icon, title, children, tone = "neutral", className }: { icon: LucideIcon; title: string; children?: ReactNode; tone?: "neutral" | "warn" | "critical"; className?: string }) {
  return (
    <div className={cn("flex flex-col items-center justify-center text-center rounded-[var(--radius-lg)] border border-dashed border-border-strong bg-inset px-6 py-10", className)}>
      <div className={cn("mb-3 grid size-11 place-items-center rounded-full", tone === "warn" ? "bg-warn-soft text-warn-fg" : tone === "critical" ? "bg-critical-soft text-critical-fg" : "bg-elevated text-fg-muted border border-border")}>
