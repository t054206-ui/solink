...
export function Badge({ tone = "neutral", className, children, icon, title }: { tone?: Tone; className?: string; children: ReactNode; icon?: ReactNode; title?: string }) {
  return (
    <span title={title} className={cn("inline-flex items-center gap-1 rounded-[var(--radius)] border px-1.5 py-px text-[11px] font-medium leading-[16px] whitespace-nowrap", tones[tone], className)}>
      {icon}{children}
    </span>
  );
}
