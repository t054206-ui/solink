...
const tones: Record<Tone, string> = {
  neutral: "bg-inset text-fg-secondary border-[var(--brass)]",
...
export function Badge({ tone = "neutral", className, children, icon, title }: { tone?: Tone; className?: string; children: ReactNode; icon?: ReactNode; title?: string }) {
  return (
    <span title={title} className={cn("inline-flex items-center gap-1 rounded-[3px] border px-1.5 py-0.5 text-[11px] font-medium leading-4 whitespace-nowrap", tones[tone], className)}>
      {icon}{children}
    </span>
  );
}
